...
recipients = ["+265998006237", "+265995246144"]
cluster = get_xi_data('http://10.44.0.52:8000/sites/api/v1/get_single_cluster/17')
...
for site_id in cluster['site']:
    ...
    while (count < 3):
        ...
        if subprocess.call(['ping', param, '1', site['ip_address']]) == 0:
            ...
            api_version = sp.getoutput('cd /var/lib/jenkins/workspace/poc-pipeline-23-srv1-1/BHT-EMR-API && git describe')
            print (api_version)


            if api_version == version:
                msgx = "Hi there,\n\nDeployment of API to " + version + " for " + site['name'] + " completed succesfully.\n\nThanks!\nEGPAF/LIN HIS."
                deployed_site= "" +site['name'] + "\n"
                with open("completed.txt", "a") as f:
                    for word in deployed_site:
                                f.writelines(word)
            else:
                msgx = "Hi there,\n\nSomething went wrong while checking out to the latest API version. Current version is " + version + " for " + site['name'] + ".\n\nThanks!\nEGPAF/LIN HIS."
                not_deployed_site= "" +site['name'] + "\n"
                with open("not_completed.txt", "a") as f:
                    for word in not_deployed_site:
                                f.writelines(word)
            ...
            if count == 3:
                failled_site= "" +site['name'] + "\n"
                with open("unreachable_sites.txt", "a") as f:
                    for word in failled_site:
                                f.writelines(word)

                for recipient in recipients:
                    msg = "Hi there,\n\nDeployment of API to {apiversion}  for " + site['name'] + " failed to complete after several connection attempts.\n\nThanks!\nEGPAF/LIN HIS."
                    params = {
                        "api_key": os.getenv('API_KEY'),
                        "recipient": recipient,
                        "message": msg
